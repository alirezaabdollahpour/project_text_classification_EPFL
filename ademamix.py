...
def alpha_scheduler(alpha_end, alpha_start=0.0, warmup=1):
    """
    Returns a callable mapping step -> alpha value with linear warmup.
    """
    def schedule(step: int):
        if warmup <= 0:
            return alpha_end
        a = min(step / float(warmup), 1.0)
        return (1.0 - a) * alpha_start + a * alpha_end
    return schedule


def beta3_scheduler(beta_end, beta_start=0.0, warmup=1):
    """
    Returns a callable mapping step -> beta3 value using half-life interpolation.
    """
    def f(beta, eps=1e-8):
        return math.log(0.5)/math.log(beta+eps)-1
...
class AdEMAMix(Optimizer):
    ...
    Arguments:
        ...
        beta3_scheduler (callable, optional): function mapping step -> beta3 value (overrides beta3_warmup if provided)
        alpha_scheduler (callable, optional): function mapping step -> alpha value (overrides alpha_warmup if provided)
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999, 0.9999), alpha=2.0, 
                 beta3_warmup=None, alpha_warmup=None, beta3_scheduler=None, alpha_scheduler=None, eps=1e-8,
                 weight_decay=0):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, alpha=alpha, beta3_warmup=beta3_warmup,
                        alpha_warmup=alpha_warmup, beta3_scheduler=beta3_scheduler, alpha_scheduler=alpha_scheduler,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            beta3_schedule_fn = group.get("beta3_scheduler", None)
            alpha_schedule_fn = group.get("alpha_scheduler", None)
            ...
            for p in group['params']:
                ...
                if alpha_schedule_fn is not None:
                    alpha = alpha_schedule_fn(state["step"])
                elif alpha_warmup is not None:
                    alpha = linear_warmup_scheduler(state["step"], alpha_end=alpha_final, alpha_start=0, warmup=alpha_warmup)
                else:
                    alpha = alpha_final
                
                if beta3_schedule_fn is not None:
                    beta3 = beta3_schedule_fn(state["step"])
                elif beta3_warmup is not None:
